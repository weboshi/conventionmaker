...
export default class EditEvents extends Component {
  ...
  mapEvents = () => {
    ...
    return events.map((event, i) =>
        ...
        <Card.Body>
        <Card.Title>{event.eventHeader}</Card.Title>
        <Card.Text>
            {event.eventBlurb}
        </Card.Text>
        </Card.Body>
            {((event.eventImage !== null) || (event.eventImage !== undefined)) && <div className="event-image"><img min-height='100px' src={imageUrls[i]}></img></div>}
        </Card>
        )
        }
    ...
    return (
      <div className="Create-Convention">
        ...
        <div className="alert-section">
            <AlertComponent success={this.state.success} successAlert={this.state.successAlert} handleDismiss={this.handleDismiss} show={this.state.showAlert}/>
        </div>
        <div className="faq-panel">{this.state.events !== undefined && this.eventsPanel()}</div>
        <ModalComponent 
          id={"showDelete"} 
          size={"sm"} 
